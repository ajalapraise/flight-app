...
export function FlightCard({ flight, classAvailability }: FlightCardProps) {
  const hasAvailability =
    classAvailability.economy + classAvailability.business + classAvailability.first > 0;

  return (
    <article className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wide text-slate-500">{flight.flight_no}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 text-sm text-slate-700">
        ...
        <div>
          <span className="font-medium">{flightDuration(flight.departs_at, flight.arrives_at)}</span>
